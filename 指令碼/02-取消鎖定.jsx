...
function unLockObj(){
    for( pv1=0 ; pv1<round ; pv1++ ){ // 重複做滿指定的次數
        win = new Window ("palette",scriptName);
        var txt = win.add("staticText");
        txt.text = inDoc.pageItems.length+"個物件進行中...";
        progress = progressBarMaker(win,inDoc.pageItems.length);
        for( pv2=inDoc.pageItems.length-1 ; pv2>=0 ; pv2-- ){
            progress.value = inDoc.pageItems.length-pv2+1;
