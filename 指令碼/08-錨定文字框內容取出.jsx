...
app.scriptArgs.set(  "loopmax" , "4" );
var loopmax;//= "6";

var win = new Window("palette");//進度框
var progress = null;//進度條
...
function flatAnchorText(){
    ...
    //countTfWithTf().length !=0 && loopCount<loopmax
    while( loopCount<loopmax ){
        var tfAry = getTfCollect(); // 尋找內容不含物件的文字框
        //因為必須倒著逐一物件工作，必須先收集物件再由最後做到第一個(根據經驗前面動，會影響後面物件相關位置及數量最後面幾個一定會錯誤)
        if(tfAry.length==0){
            return null;
        }
        win = new Window("palette");
        win.add("staticText").text = "第" + (loopCount+1) + "輪篩選結果: " + tfAry.length + "個可疑文字框作業中...請耐心等候!!";
        progress = progressBarObj(win,tfAry.length);
        for( var pv2=tfAry.length-1 ; pv2>=0 ; pv2-- ){
            progress.value = tfAry.length-pv2+1;
...
function getTfCollect(){
    ...
    win = new Window("palette");
    win.add("staticText").text = "篩選" + inDoc.allPageItems.length + "個物件中...";
    progress = progressBarObj(win,inDoc.allPageItems.length);
	for( var v2=inDoc.allPageItems.length-1 ; v2>=0 ; v2-- ){
        progress.value = inDoc.allPageItems.length-v2+1;
        //$.sleep(200);
...
//進度條
function progressBarObj(parentWin,stopInt)
{
