...
function getGroupContents(){
    ...
    for( pv1=pageObjs.length-1 ; pv1>=0 ; pv1-- ){
        win1Label1.text = "識別群組物件中.....["+(pv1+1)+"]/["+pageObjs.length+"]";
        progress1.value = pageObjs.length-pv1+1;
    ...
    var progress1 = progressBarMaker(win1,pGroupAry.length);//進度條
    for( pv2=pGroupAry.length-1 ; pv2>=0 ; pv2-- ){
        win1Label1.text = "群組物件批次作業.....["+(pv2+1)+"]/["+pGroupAry.length+"]";
        progress1.value = pGroupAry.length-pv2+1;
        ...
        if( pGroupAry[pv2].parent.constructor.name == "Character" ){ // 如果爸爸是文字流中的一分子
            ...
            win1Label2.text = "群組物件編號：["+pGroupAry[pv2].id+"] 進行中.....   錨定字元內容："+parentChar.contents+"  錨定字元插入點數量："+parentChar.insertionPoints.length;
            if( parentChar.insertionPoints.length==2 ){// 必須有前後插入點
                parentChar.insertionPoints[0].contents = norStr;
            }
