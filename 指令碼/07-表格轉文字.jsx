...
function flatTable(){
    for( pv1=inDoc.stories.length-1 ; pv1>=0 ; pv1-- ){
        ...
        while( inStory.tables.length != 0 && loopCount<loopmax ){
            var win1 = new Window("palette");//進度窗
            var win1Label1 = staticTextMaker(win1);//進度訊息
            win1Label1.text = "本文：["+ (pv1+1)+"] / ["+inDoc.stories.length+"]";
            var win1Label2 = staticTextMaker(win1);//進度訊息            
            var progress1 = progressBarMaker(win1,inStory.tables.length);//進度條
            for( pv2=0 ; pv2<inStory.tables.length ; pv2++ ){
                win1Label2.text = "表格：["+(pv2+1)+"]/["+inStory.tables.length+"]";
                progress1.value = pv2+1;//進度+1                
