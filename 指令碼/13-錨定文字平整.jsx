...
function flatAnchorText(){
    ...
    while( countTfWithTf().length !=0 && loopCount<loopmax ){
        ...
        var progress1 = progressBarMaker(win1,tfAry.length);//進度條 
        for( var pv2=tfAry.length-1 ; pv2>=0 ; pv2-- ){
            win1Label1.text = "文字框處理中.....["+(pv2+1)+"]/["+tfAry.length+"]";
            progress1.value = tfAry.length-pv2+1;
...
function getTfCollect(){
	...
	for( var v2=inDoc.allPageItems.length-1 ; v2>=0 ; v2-- ){
        win1Label1.text = "篩選可處理文字框：["+(v2+1)+"]/["+inDoc.allPageItems.length+"]";
        progress1.value = inDoc.allPageItems.length-v2+1;
...
function countTfWithTf(){
    ...
    var progress1 = progressBarMaker(win1,inDoc.allPageItems.length);//進度條
    for( v1=0 ; v1<inDoc.allPageItems.length ; v1++ ){ // 從文件的所有頁面物件開始
        win1Label1.text = "計算文框內含有文框的物件數量.....["+(v1+1)+"]/["+inDoc.allPageItems.length+"]";
        progress1.value = v1+1;
        var inObj = inDoc.allPageItems[v1];
        var typeName = inObj.constructor.name; // 取得物件型別
        if( typeName=="TextFrame" ){
            if( inObj.textFrames.length ){
                tfWithTfAry.push( inObj );// 尚有包含物件的算進去
                win1Label2.text = "累計數量："+tfWithTfAry.length;
            }
        }
    }
    progress1.parent.close();
