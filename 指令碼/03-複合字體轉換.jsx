...
function replSingleFont( userDefineFontName ){
    ...
    var win1 = new Window("palette",scriptName+"=>"+userDefineFontName);
    var winLabel1 = win1.add("statictext");//第一標籤
    winLabel1.preferredSize = [500,30];
    winLabel1.text = "識別複合字體中.....";
    var winLabel2 = win1.add("statictext");//第二標籤
    winLabel2.preferredSize = [500,30];
    winLabel2.text = "";
    var progress1 = progressBarMaker(win1,inFonts.length);//進度條
    for( var pv1=0 ; pv1<inFonts.length ; pv1++ ){
        ...
        if( validCompositeFont( inFont )==true ){ // 驗證是否為合法有效字體
             if( inFont.fontType=="1718894932" ){  // 複合字
                 winLabel1.text = "複合字體: ["+inFont.name+"] 執行中.....";
                ...
                if( entFont != null ){ // 如果傳回不為null
                    ...
                    winLabel2.text = "[" + inFont.name + "]：內含字類：[" + userDefineFontName + "] 被指定為：[" + entFont + "]";
